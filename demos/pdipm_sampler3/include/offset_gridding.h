 ...
 * @Date: 2022-08-11 11:43:50
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2022-08-23 19:50:38
  ...
  public:
    ...
    std::tuple<std::vector<particle>, std::vector<int>> getDataByRed(int hash)
    {
      ...
      for (auto i = 0; i < mData.size(); i++)
      {
        ...
        if ((grid_xyz.x + grid_xyz.y + grid_xyz.z) % 2 == 0 && std::find(child_hash.begin(), child_hash.end(), data_grid_hash) !=
                                                                   child_hash.end())
        {
    ...
    std::tuple<std::vector<particle>, std::vector<int>> getDataByBlack(int hash)
    {
      ...
      for (auto i = 0; i < mData.size(); i++)
      {
        ...
        auto grid_xyz = computeGridXYZByPos3(rel_pos);
        auto data_grid_hash = computePos2GridHash(mData[i].pos);
        if ((grid_xyz.x + grid_xyz.y + grid_xyz.z) % 2 == 1 && std::find(child_hash.begin(), child_hash.end(), data_grid_hash) !=
                                                                   child_hash.end())
        {
          index.emplace_back(i);
          data.emplace_back(mData[i]);
