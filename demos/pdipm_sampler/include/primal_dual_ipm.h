 ...
 * @LastEditTime: 2022-07-18 13:08:22
...
dual2nd InEquConstrainstFunc(const VectorXdual2nd &x, const MatrixXd &lambda,
                             const std::vector<particle> &p, int equNum)
{
  auto counter = equNum;
  auto n = x.size();
  dual2nd sum = 0.0;
...
namespace OPTIMIZE::IPM
{
  ...
  public:
    explicit PrimalDualIPM(const std::vector<double> &data,
                           const std::vector<particle> &particles, int equNum,
                           int inEquNum)
    ...
    void solve()
    {
      ...
      for (auto i = 0; i < mOuterIterNum; i++)
      {
        ...
        {
          KIRI_LOG_INFO("Progress(%)={0}", static_cast<double>(i * mInnerIterNum + j) / static_cast<double>(mInnerIterNum * mOuterIterNum));
  ...
  private:
    ...
    double computeMeritFunction(VectorXreal realData, VectorXdual2nd dualData,
                                MatrixXd slack)
    {
      auto phi_nu = eval(TargetFunc(dualData, mParticles));
    ...
    void computeConstraintsJacobian()
    {
      ...
      mFuncHessian = hessian(TargetFunc, wrt(mDual2ndData),
                             at(mDual2ndData, mParticles), u, mFuncGrad);
    ...
    void computeLagrangianHessian()
    {
      ...
      if (mInEquNum > 0)
      {
        ...
        VectorXdual g_ci;
        MatrixXd d2ci =
            hessian(InEquConstrainstFunc, wrt(mDual2ndData),
                    at(mDual2ndData, mLambda, mParticles, mEquNum), u_ci, g_ci);
