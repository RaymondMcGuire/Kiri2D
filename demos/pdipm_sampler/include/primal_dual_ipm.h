 ...
 * @Date: 2022-09-07 13:51:33
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2022-09-07 17:47:07
...
namespace OPTIMIZE::IPM
{
  ...
  private:
    ...
    void computeLagrangianHessian()
    {
      ...
      // KIRI_LOG_INFO("w={0};{1};{2};{3}", w.array().abs().minCoeff(), w.array().abs().maxCoeff(), w.array().size(), mInEquNum + mEquNum);

      // SpMat hess_sparse = hess.sparseView();

      // // Construct matrix operation object using the wrapper class SparseGenMatProd
      // SparseGenMatProd<double> op(hess_sparse);

      // // Construct eigen solver object, requesting the largest three eigenvalues
      // GenEigsSolver<SparseGenMatProd<double>> eigs(op, mInEquNum + mEquNum, 2 * (mInEquNum + mEquNum) + 1);
      ...
      //           << evalues.array().abs().minCoeff() << ";" << evalues.array().abs().maxCoeff() << std::endl;
