                             ...
                             int inEquNum)
{
  ...
  for (auto i = 0; i < n; i++)
  {
    ...
    {
      consts[counter++] = (p[i].pos - allParticles[p[i].neighbors[j]].pos).length() - (x[i] + allParticles[p[i].neighbors[j]].radius);
    }
  }
                             ...
                             int equNum)
{
  ...
  for (auto i = 0; i < n; i++)
  {
    for (auto j = 0; j < p[i].neighbors.size(); j++)
    {
      sum += ((p[i].pos - allParticles[p[i].neighbors[j]].pos).length() - (x[i] + allParticles[p[i].neighbors[j]].radius)) * lambda(counter++, 0);
    }
  }
