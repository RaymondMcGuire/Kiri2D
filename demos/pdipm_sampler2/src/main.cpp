...
int main(int argc, char *argv[]) {
  ...
  // 3, 3, 3); auto grid_size = gridding->maxGridHash(); for (auto i = 0; i <
  // grid_size; i++)
  // {
  ...
  for (auto iter = 0; iter < 50; iter++) {
    for (auto i = 0; i < grid_size; i++) {
      ...
      // for (auto idx = 0; idx < particles_index.size(); idx++) {
      //   auto neighbors = grid_particles[idx].neighbors;
      //   KIRI_LOG_DEBUG("idx={0}; pos={1},{2},{3}", particles_index[idx],
      //                  grid_particles[idx].pos.x, grid_particles[idx].pos.y,
      //                  grid_particles[idx].pos.z);

      //   for (auto j = 0; j < neighbors.size(); j++) {
      //     KIRI_LOG_DEBUG("neighbor idx={0}; pos={1},{2},{3}", neighbors[j],
      //                    data_particles[neighbors[j]].pos.x,
      //                    data_particles[neighbors[j]].pos.y,
      //                    data_particles[neighbors[j]].pos.z);
      //   }
      // }
      ...
      //       grid_particles[j].pos.x / scale, grid_particles[j].pos.y / scale,
      //       grid_particles[j].pos.z / scale, grid_particles[j].radius /
      //       scale));
      // }

      // ExportBgeoFileFromCPU("box", "box_opti_" + std::to_string(i),
      // positions);
    }
