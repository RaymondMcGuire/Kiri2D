 ...
 * @Date: 2022-08-23 22:50:48
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2022-08-24 13:43:32
...
int main(int argc, char *argv[]) {
  ...
  //   // offset_gridding
  //   // auto offset_gridding =
  //   std::make_shared<OPTIMIZE::IPM::OffsetGridding>(
  ...
  //         data_particles[j].pos.z / scale, data_particles[j].radius /
  //         scale));
  //   }
  ...
  for (auto iter = 0; iter < 20; iter++) {
    auto offset_gridding =
        std::make_shared<OPTIMIZE::IPM::Gridding>(data_particles, 3, 3, 3);
    ...
    for (auto i = 0; i < offset_grid_size; i++) {
      auto boundary_constrains_num = 0;
      auto [grid_particles, particles_index] = offset_gridding->getDataByRed(i);
      n = grid_particles.size();
