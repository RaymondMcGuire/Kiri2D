 ...
 * @LastEditTime: 2022-12-01 23:21:18
...
int main(int argc, char *argv[]) {
  ...
  for (auto i = 0; i < sampler_num; i++) {
    ...
    sites.emplace_back(
        std::make_shared<HDV::Primitives::Vertex2>(rnd_point.x, rnd_point.y));
  }
  ...
  for (auto i = 0; i < simplexs.size(); i++) {
    auto line = KiriLine2(Vector2F(simplexs[i].x, simplexs[i].y) + offset,
                          Vector2F(simplexs[i].z, simplexs[i].w) + offset);
    line.thick = 1.f;
