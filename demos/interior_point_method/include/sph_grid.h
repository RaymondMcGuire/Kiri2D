 ...
 * @LastEditTime: 2022-07-12 14:46:41
 * @FilePath: \Kiri2D\demos\interior_point_method\include\sph_grid.h
 * @Description: 
...
namespace OPTIMIZE::IPM
{
    ...
    public:
         ...
         std::vector<Cell> getNeighboringCells(Vector3D position)
        {
            ...
            if (xCell < numberCells.x - 1 &&yCell < numberCells.y - 1 && zCell < numberCells.z - 1)
        ...
        {
            cells = std::vector<std::vector<std::vector<Cell>>>(numberCells.x, std::vector<std::vector<Cell>>(numberCells.y,std::vector<Cell>(numberCells.z ,Cell())));
