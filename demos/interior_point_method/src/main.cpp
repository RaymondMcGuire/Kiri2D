 ...
 * @LastEditTime: 2022-07-14 12:34:29
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < neighborhoods.size(); i++) {
    ...
    auto ipm = std::make_shared<OPTIMIZE::IPM::InteriorPointMethod>(
        data, inequ_num, tmp_particles);
  ...
  // inequ_num, data_radius, data_pos); auto results = ipm->solution();

  // for (auto j = 0; j < n; j++)
  // {
  //     // if (data_radius[j] / scale * double(results[j]) >= 1e-4)
  //     positions.emplace_back(Vector4D(data_pos[j].x / scale, data_pos[j].y /
  //     scale, data_pos[j].z / scale, data_radius[j] / scale *
  //     double(results[j]))); KIRI_LOG_DEBUG("radius={0};{1}", data_radius[j] /
  //     scale * double(results[j]), double(results[j]));
  // }
