 ...
 * @LastEditTime: 2022-12-01 23:22:36
...
int main(int argc, char *argv[]) {
  ...
  auto offset = (Vector2F((size_t)window_width, (size_t)window_height) -
                ...
                2.f;
  sampler->initUniRadiiSampler(
      radius, Vector2F(boundary_bbox.width(), boundary_bbox.height()));
  ...
  while (1) {
    ...
    auto uni_points = sampler->generateUniRadii(
        radius, Vector2F(boundary_bbox.width(), boundary_bbox.height()));
    ...
    for (auto i = 0; i < uni_points.size(); i++)
      if (boundary_polygon->contains(
              Vector2D(uni_points[i].x, uni_points[i].y)))
