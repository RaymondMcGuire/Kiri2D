 ...
 * @LastEditTime: 2022-10-26 16:53:44
 * @FilePath: \Kiri2D\demos\test_algo\src\main.cpp
 * @Description:
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        for (auto i = 0; i < all_circles.size(); i++)
        {
            ...
            {
                q_c = Vector2D(point.x, point.y) + point.z * (rnd_point - Vector2D(point.x, point.y)).normalized();
            }
        }

        auto epsilon = cooling_func(iter_num, 0.08, 0.8);
        auto current_move = epsilon * (rnd_point - q_c);
        ...
        if (current_move_len < 1e-6)
        {
            ...
            all_circles.emplace_back(Vector3D(rnd_point.x, rnd_point.y, rnd_point.distanceTo(q_c)));
            all_circles_draw.emplace_back(KiriCircle2(Vector2F(rnd_point.x, rnd_point.y) + offset, Vector3F(Random::get(0.0, 1.0), Random::get(0.0, 1.0), Random::get(0.0, 1.0)), rnd_point.distanceTo(q_c), true));

            auto pos_flag = false;
            while (!pos_flag)
            {
                pos_flag = true;
                rnd_point = boundary->rndInnerPoint();
                for (auto i = 0; i < all_circles.size(); i++)
                {
                    auto point = all_circles[i];
                    auto dist = rnd_point.distanceTo(Vector2D(point.x, point.y)) - point.z;
        ...
        {
            circles.emplace_back(KiriCircle2(Vector2F(rnd_point.x, rnd_point.y) + offset, Vector3F(0.f, 1.f, 1.f), rnd_point.distanceTo(q_c), false));
        }
