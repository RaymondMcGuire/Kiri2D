...
private:
  ...
  void initLambdaMultipler() {
    ...
    MatrixXd lambda =
        mConstrainJacobian.completeOrthogonalDecomposition().pseudoInverse() *
  ...
  void computeConstrainstsJacobian() {
    ...
    MatrixXd constrainst_jacobian_with_identity(mConstrainJacobian.rows() +
                                                    eyeConstrainMatrix.rows(),
                                                mConstrainJacobian.cols());
    constrainst_jacobian_with_identity << mConstrainJacobian,
        eyeConstrainMatrix;
  ...
  void computeBackTrackingLineSearch(MatrixXd dz, double alphaSMax = 1.0,
    ...
    if (phi_nu1 > phi1_next) {
      ...
      if (con_new_sum > con_old_sum) {
        ...
        auto ci2 = this->computeConstrainsts(
