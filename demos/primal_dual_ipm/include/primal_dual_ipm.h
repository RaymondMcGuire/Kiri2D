...
dual2nd InEquConstrainstFunc(const VectorXdual2nd &x, const MatrixXd &lambda,
                             const int equNum) {
  dual2nd sum;
  sum += (x[2] - x[0] * x[0]) * lambda(equNum + 0, 0);

  return sum;
}

dual2nd TargetFunc(const VectorXdual2nd &x) { return 4 * x[1] - 2 * x[2]; }
...
class PrimalDualIPM {
public:
  explicit PrimalDualIPM(const std::vector<double> &data, int equNum,
                         int inEquNum)
      : mData(data), mVariableNum(data.size()), mEquNum(equNum),
        mInEquNum(inEquNum) {
    this->initDataVector();
    this->initSlack();
...
private:
  ...
  double computeMeritFunction(VectorXreal realData, VectorXdual2nd dualData,
                              MatrixXd slack) {
    auto phi_nu = eval(TargetFunc(dualData));
  ...
  void computeConstraintsJacobian() {
    dual2nd u;
    mFuncHessian =
        hessian(TargetFunc, wrt(mDual2ndData), at(mDual2ndData), u, mFuncGrad);
