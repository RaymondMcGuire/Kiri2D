...
SweepEvent::SweepEvent(bool b, const Point_2 &p, SweepEvent *other, PolygonType pt, EdgeType et) : left(b), point(p), otherEvent(other), pol(pt), type(et), prevInResult(0), inResult(false)
{
}
...
bool BooleanOpImp::run()
{
	...
	while (!eq.empty())
	{
		...
		if ((operation == INTERSECTION && se->point.x() > MINMAXX) ||
			(operation == DIFFERENCE1 && se->point.x() > subjectBB.xmax()))
		{
			connectEdges();
			return true;
		}
		...
		{ // the line segment must be inserted into sl
			...
			if (next != sl.end())
			{
				auto res = possibleIntersection(se, *next);
				if (res == -1)
					flag = false;
			...
			if (prev != sl.end())
			{
				...
				auto res = possibleIntersection(*prev, se);
				if (res == -1)
					flag = false;
