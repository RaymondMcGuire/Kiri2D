...
namespace cbop
{
	...
	struct SegmentComp
	{
		typedef SweepEvent *first_argument_type;

		typedef SweepEvent *second_argument_type;
	...
	struct SweepEvent
	{
		...
		bool below(const Point_2 &p) const { return (left) ? signedArea(point, otherEvent->point, p) > 0 : signedArea(otherEvent->point, point, p) > 0; }
	...
	inline bool compute(const Polygon &subj, const Polygon &clip, Polygon &result, BooleanOpType op)
	{
