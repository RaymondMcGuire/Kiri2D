...
__host__ __device__ inline float lengthSquared(float2 v) { return dot(v, v); }
...
inline __host__ __device__ float2 sgn(float2 v) {
  return make_float2(sgn(v.x), sgn(v.y));
}
...
inline __host__ __device__ float3 f2tf3(float2 f) {
  return make_float3(f.x, f.y, 0.f);
}
