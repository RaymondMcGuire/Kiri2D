/*
 * File: kiri2d.h
 * Module: include
 * Created Date: 2025-08-18
 * Author: Xu WANG
 * -----
 * Last Modified: 2025-08-19
 * Modified By: Xu WANG
 * -----
 * Copyright (c) 2025 Xu WANG
 */

#ifndef _KIRI2D_H_
#define _KIRI2D_H_

#pragma once

#include <kiri2d/renderer/renderer.h>
#include <kiri_log.h>

#include <kiri2d/hdv_toolkit/sampler/ms_sampler2.h>
#include <kiri2d/hdv_toolkit/sampler/poisson_disk_sampler.h>
#include <kiri2d/hdv_toolkit/voronoi/voronoi_nocaj_treemap.h>

#include <kiri2d/proto_sphere/proto_sphere_bon20.h>
#include <kiri2d/proto_sphere/proto_sphere_opti.h>
#include <kiri2d/proto_sphere/proto_sphere_packing.h>
#include <kiri2d/proto_sphere/proto_sphere_packing_sdf.h>
#include <kiri2d/proto_sphere/proto_sphere_packing_sdf_opti.h>

#include <kiri2d/proto_sphere/soft_protosphere_TR.h>
#include <kiri2d/proto_sphere/soft_protosphere_standard.h>

#include <kiri2d/physics/rigidbody/rigidbody_system.h>

#endif