...
namespace HDV::SDF
{
  ...
  public:
    ...
    std::vector<Vector2D> placeRndPoints(int num) const
    {
      std::vector<Vector2D> interiorPoints;

      while (static_cast<int>(interiorPoints.size()) < num)
      {
        // Random point in bounding box

        Vector2D rndPoint = Vector2D(Random::get(0.0, 1.0) * mBbox.width(),
                                     Random::get(0.0, 1.0) * mBbox.height()) +
                            mBbox.LowestPoint;

        const auto [sdfValue, _] = getSDF(rndPoint);

        // Add point if inside polygon
        if (sdfValue > 0.0)
        {
          interiorPoints.emplace_back(rndPoint);
        }
      }
