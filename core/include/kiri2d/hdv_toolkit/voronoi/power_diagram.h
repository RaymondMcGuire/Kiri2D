...
namespace HDV::Voronoi
{
    ...
    public:
        ...
        bool generateRndSites(int num = 100)
        {
            auto polygon = this->boundary();
            if (polygon)
            {
                for (auto i = 0; i < num; i++)
                {
                    auto point = polygon->rndInnerPoint();
                    this->addSite(std::make_shared<Voronoi::VoronoiSite2>(point.x, point.y, i));
                }
            }
            else
            {
                KIRI_LOG_ERROR("PowerDiagram2D: Please set boundary polygon first!");
                return false;
            }
    ...
    public:
        ...
        const std::shared_ptr<VoronoiPolygon3> &boundary()
        {
