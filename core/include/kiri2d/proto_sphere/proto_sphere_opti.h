...
public:
  explicit ProtoSphereOpti(const HDV::Voronoi::VoronoiPolygon2Ptr &boundary,
                           std::vector<double> target_radius)
      : ProtoSpherePacking2D(boundary), mTargetRadiusArray(target_radius) {
    std::sort(mTargetRadiusArray.begin(), mTargetRadiusArray.end(),
              std::greater<double>());
    mRadiusMax = mTargetRadiusArray[0];
    mRadiusMin = mTargetRadiusArray[mTargetRadiusArray.size() - 1];
  ...
  virtual void convergePrototype() override {
    ...
    for (auto i = 0; i < mInsertedSpheres.size(); i++) {
      ...
      if (dist < q_c.distanceTo(mCurrentSphere))
        q_c = Vector2D(point.x, point.y) +
              point.z *
                  (mCurrentSphere - Vector2D(point.x, point.y)).normalized();
    }
    ...
    // KIRI_LOG_DEBUG("current radius ={0}; target radius ={1}",
    // mCurrentSphereRadius, mCurrentTargetRadius);

    if (abs(radius_dist) < mCurrentTargetRadius * mErrorRate ||
        mIterNum > mMaxSingleIter) {
      mIterNum = 0;

      // check satifty the min radius

      mInsertedSpheres.emplace_back(
          Vector3D(mCurrentSphere.x, mCurrentSphere.y, mCurrentSphereRadius));
      mInsertedSpheresColor.emplace_back(Vector3F(
          Random::get(0.0, 1.0), Random::get(0.0, 1.0), Random::get(0.0, 1.0)));
      ...
      if (mInsertedSpheres.size() < mCurrentTargetRadiusArray.size()) {
        ...
        while (!pos_flag) {
          pos_flag = true;
          mCurrentSphere =
              mBoundary->rndInnerPointWithDistConstrain(mRadiusMin);
          for (auto i = 0; i < mInsertedSpheres.size(); i++) {
            auto point = mInsertedSpheres[i];
            auto dist =
                mCurrentSphere.distanceTo(Vector2D(point.x, point.y)) - point.z;
            ...
            if (sample_iter_num > mMaxSingleIter * 10)
              constrain_error_rate =
                  1.0 - mErrorRate * (sample_iter_num / mMaxSingleIter);
...
private:
  ...
  void ReAllocateTargetRadius() {
    mCurrentTargetRadiusArray = std::vector<double>(
        mTargetRadiusArray.begin(),
        mTargetRadiusArray.begin() + mTargetRadiusArray.size());
