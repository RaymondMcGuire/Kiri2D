/*
 * File: proto_sphere_packing_sdf_opti.h
 * Module: proto_sphere
...
public:
  explicit ProtoSpherePackingSDFOpti(
      ...
      const std::vector<double> radiusRangeProb, bool useRandomOffset,
  ...
  void statisticDistribution() {
    ...
    for (auto ri = 0; ri < mPreDefinedRadiusRange.size() - 1; ri++) {
      ...
      distribution_str += std::to_string((int)mPreDefinedRadiusRange[ri]) +
                          "---" +
    ...
    KIRI_LOG_DEBUG("{0}; porosity={1}; rmspe={2}; bhatta_val={3}",
                   distribution_str, 1.0 - v / mBoundary->area(), rmspe,
                   bhatta_val);
  ...
  virtual bool convergePrototype() {
    ...
    for (auto i = 0; i < mCurrentSpheres.size(); i++) {
      ...
      Vector2D q_c;
      if (mUseRandomOffset) {
        std::tie(current_radius, q_c) = mSDF2D->getSDFWithRndOffset(pos);
      } else {
        std::tie(current_radius, q_c) = mSDF2D->getSDF(pos);
      }
    ...
    if (mAllConverged && !mDrawCurrentSpheres) {
      ...
      for (auto i = 0; i < mCurrentSpheres.size(); i++) {
        ...
        if (!overlapping &&
          ...
          if ((mRemainSamples[cur_idx] * mInsertedIntervalNum) >=
                  mTotalCounter[cur_idx] ||
              mStartFindMinimumPorosity) {
