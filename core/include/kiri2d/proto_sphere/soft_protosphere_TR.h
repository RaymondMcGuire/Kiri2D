...
namespace PSPACK
{
  ...
  protected:
    ...
    {
      // 1) 取各约束的距离，并找 d0
      auto [d_poly, _tag] = mSDF2D->getSDF(pos);

      // 边界法向（建议单位化）
      Vector2D grad_poly = mSDF2D->getSDFGradient(pos);
      const double gpn = grad_poly.length();
      if (gpn > 1e-12)
        grad_poly /= gpn;

      double d0 = d_poly;
      for (const auto &s : mInsertedSpheres)
      {
        const Vector2D c(s.x, s.y);
        const double dist = (pos - c).length();
        const double d = dist - s.z;
        if (d < d0)
          d0 = d;
      }

      // 2) 基于 d0 计算稳定的 soft-min 权重并累加梯度
      Vector2D grad(0.0, 0.0);
      double wsum = 0.0;

      // 边界项
      const double w_poly = std::exp(-(d_poly - d0) / mTau);
      wsum += w_poly;
      grad += w_poly * grad_poly;
      ...
      for (const auto &s : mInsertedSpheres)
      {
        ...
        const double d = dist - s.z;
        const double w = std::exp(-(d - d0) / mTau);
        const Vector2D grad_s = diff / dist; // 单位外法向
