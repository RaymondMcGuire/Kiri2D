...
#ifndef _SOFT_PROTO_SPHERE_STANDARD_2D_H_
#define _SOFT_PROTO_SPHERE_STANDARD_2D_H_
...
namespace PSPACK
{
  class SoftProtoSphereStandard2D
  {
  public:
    static constexpr int MAX_ITERATIONS = 150;
    ...
    static constexpr double DEFAULT_TAU =
        50; // Temperature parameter for soft-min
    ...
    explicit SoftProtoSphereStandard2D(
    ...
    virtual ~SoftProtoSphereStandard2D() = default;
  ...
  protected:
    void initParticles()
    {
      ...
      // auto data = mSDF2D->placeRndPoints(1);
    ...
    Vector2D computeSoftMinGradient(const Vector2D &pos) const
    {
      ...
      // printf("Boundary dist: %f, Gradient: (%f, %f)\n", d_poly, grad_poly.x,
      //        grad_poly.y);

      double d0 = d_poly;
      for (const auto &sphere : mInsertedSpheres)
      {
        Vector2D c(sphere.x, sphere.y);
        double dist = (pos - c).length();
        double d = dist - sphere.z;
        if (d < d0)
          d0 = d;
      }
      if (d0 <= 0.0)
      ...
      double w_poly = std::exp(-(d_poly - d0) / mTau);
      sum_weights += w_poly;
      gradient += w_poly * grad_poly;

      // printf("Weight poly: %f, Partial gradient: (%f, %f)\n", w_poly,
      //        w_poly * grad_poly.x, w_poly * grad_poly.y);
      ...
      for (const auto &sphere : mInsertedSpheres)
      {
        ...
        double w_sphere = std::exp(-(d_sphere - d0) / mTau);
      ...
      // printf("Sum weights: %f, Gradient before normalization: (%f, %f)\n",
      //        sum_weights, gradient.x, gradient.y);
    ...
    void updateSpherePositionWithLineSearch(size_t index)
    {
      ...
      if (gradient.length() < CONVERGENCE_THRESHOLD)
      {
        ...
        // printf("Check convergence based on gradient magnitude! Sphere %zu converged with radius %f\n", index,
        //        sphere.z);
        return;
      }
      ...
      {
        // printf("Check convergence based on position change! Sphere %zu converged with radius %f\n", index,
        //        sphere.z);
        mConverges[index] = true;
      }
    }
